                    ...
                    if (userList.size() == 1) {
                        ...
                        if (userId.equals(userList.get(0).getUserID())) {
                            if (body.toString().contains("error")) {
                                ...
                                infoLog = String.format("decode1>>>  %s",decode);
                                logger.info(infoLog);
                                ApiToken apiTokenObj = new ApiToken();
                                if (userList.size() == 1) {
                                    logger.info(">>>>userList.size()==1<<<<");
                                    logger.info(">>>>body.toString():::"+body.toString());
                                    trnJsonObjResponse.setResponseTime(new Date());
                        ...
                        if (userList.size() == 1) {
                            ...
                            if (userId.equals(userList.get(0).getUserID())) {
                                ...
                                if (body.toString().contains("error")) {
                                    ...
                                    String decode = deCodeJWT(body.toString());
                                    JSONObject decodeJsonObj = new JSONObject(decode);
                                    infoLog = String.format("decode2>>>  %s",decode);
                                    logger.info(infoLog);
                                    ApiToken apiTokenObj = new ApiToken();
                                    if (userList.size() == 1) {
                                        logger.info(">>>>userList.size()==1<<<<");
                        ...
                        if (userList.size() == 1) {
                            ...
                            if (userId.equals(userList.get(0).getUserID())) {
                                ...
                                if (body.toString().contains("error")) {
                                    ...
                                    String decode = deCodeJWT(body.toString());
                                    JSONObject decodeJsonObj = new JSONObject(decode);
                                    infoLog = String.format("decode3>>>  %s",decode);
                                    logger.info(infoLog);
                                    ApiToken apiTokenObj = new ApiToken();
                                    if (userList.size() == 1) {
                                        logger.info(">>>>userList.size()==1<<<<");
